		...
		<view class="content">
			<view class="list">
				...
				<view class="bigalinetea">
					<view class="left" @click="backPageRefresh">联系地址<text class="requisite">*</text>：</view>
					<view class="right">
						<textarea placeholder="请输入联系地址" class="uni-textarea" v-model="descaddress"></textarea>
					</view>
				</view>
			</view>
		</view>
	...
	export default {
		...
		methods: {
			query(id){
				...
				$.post('/erp/addressBook/update',data).then(res=>{
					console.log(res)
					if(res.code==0){
						this.name=res.data.name
						this.phone=res.data.phone
						this.descaddress=res.data.address
					}
				}).catch(err=>{
					console.log(err)
				})
			...
			nowgo() {
				...
				$.post('/erp/addressBook/edit', obj).then(res => {
					if (res.code == 0) {
						uni.showToast({
							title: "修改成功!"
						})
